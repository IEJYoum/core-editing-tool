...
print("this version fails on large tmas because cookie is too big!")
...
def getData():
    ...
    if request.method == 'POST':
        ...
        session['nrw'] = int(nr)
        session['ncl'] = int(nc)
        print(session.get('data'),"data in getdata post!")
        print(type(data))
...
def edit():
    print(session.get('coreID'),'ids 0')
    if request.method == 'POST':
        intid = request.form.get('intid')
        gridid = request.form.get('gridid')
        nam = session.get('names')
        ids = session.get('coreID')
        print(ids,"ids 1")
        ...
        ids[nind] = gridid
        session['gridid'] = ids
        session['coreID'] = ids #what here is redundant... 
        print(ids,"ids 2")
        coords = pd.DataFrame([session.get('xs'),session.get('ys')]).transpose()
        tensors = [session.get('htens'),session.get('vtens')]
...
def save():
    ...
    rounds.append(rnd)
    session["rounds"] = rounds  #done saved rounds
    return(redirect(url_for("mmenu")))
        ...
        if rd not in rounds:
            ...
            if "coremap" in list(sdf.columns):
                if list(sdf.loc[:,'coremap'])[0] != "nan":
                    session['coreID'] = list(sdf.loc[:,'coremap'])
            return(sdf)
    return("done")
...
def getRM(coords,names,title=''):   
    ...
    i = 0
    reps = 0
    if type(session.get('coreID')) == type(None):
        while True:
            try:
                i += 1
...
def showOut(coords,names,coreID,tensors,title=""):
    ...
    fig = plt.figure(figsize=(int(session.get('ncl'))/5,int(session.get('nrw'))/5),dpi=500)
...
def findClosest(coords,inters):
    ...
    for i in range(coords.shape[0]):
        ...
        colInd = int(session.get('nrw')) - (math.floor(minD/int(session.get('ncl'))))-1
        #colInd = math.floor(minD/int(session.get('ncl')))
        colL = alphab[colInd]

        rowN = (minD % int(session.get('ncl'))) +1
        rowL = str(rowN)
        if len(rowL) == 1:
            rowL = "0"+rowL
...
def setTensors(mxs,mys):
    ...
    colStep = (mxs[1] - mxs[0])/(int(session.get('ncl'))-1)
    rowStep = (mys[1] - mys[0])/(int(session.get('nrw'))-1)
    for i in range(int(session.get('nrw'))):
        b = mys[0] + i * rowStep
        tensors[0].append(torch.tensor([0.0,b],requires_grad = True))
    for i in range(int(session.get('ncl'))):
        b = mxs[0] + i * colStep
        #print(b)
